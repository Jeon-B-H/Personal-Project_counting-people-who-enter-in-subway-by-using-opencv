...
# out = cv2.VideoWriter('Mask of Presentation video 1.avi', fourcc, fps, (w, h))
...
while True:
    ...
    # Method1. Gaussian bluring을 통해 노이즈 제거 (노이즈가 심한 경우)
    # frame = cv2.GaussianBlur(frame, (0, 0), 3)

    # Method2. grayscale로 변환 후 Gaussian bluring을 적용
    frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    filtered_frame_gray = cv2.GaussianBlur(frame_gray, (0, 0), 1)

    # Method3. ROI 설정 후 MOG2 적용
    # ROI = frame[0:w, 96:384]

    for person in people:
        person.age_one() #age every person one frame

    ## MOG 배경 모델 적용 (foreground mask 두개 중에 택 1)
    fgmask1 = MOG2.apply(frame)

    ## 그림자 없애기
    try:
        # fgmask1과 fgmask2 이미지를 이진화시키기
        _, bin_fgmask1 = cv2.threshold(fgmask1, 254, 255, cv2.THRESH_BINARY)
    ...
    # out.write(mask2) # 영상 데이터만 저장. 소리는 X
